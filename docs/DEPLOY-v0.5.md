...
4. **RAG (база знаний)** — исправлен вызов API эмбеддингов в провайдере (`texts=` вместо `input=` для SDK GigaChat). Модель эмбеддингов по умолчанию: **EmbeddingsGigaR** (настраивается в конфиге). Чанки обрабатываются пакетами по 5 для стабильной работы API. Пути к файлам из Telegram подставляются в `knowledge(index_file)` через патч в loop (`[file: путь]` → `file_path`). Ограничение путей RAG только workspace при необходимости — отдельная задача.
