...
### Известные проблемы (→ v0.5)
- "Создай файл и отправь" без содержания — зацикливается на write без content
- "Пришли файл из проекта" — модель читает файл вместо отправки через message(media)
- readability-lxml не загружается на сервере (серверный баг, не код)
- RAG не протестирован полноценно (index_file, search)
